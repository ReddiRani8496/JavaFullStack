...
public class DepartmentTest {
	...
	@Test
	public void findAllDepartmentsOrderBydNameTest() {
		List<DepartmentEO> departments = departmentDAORef.findAllOrderByDepartmentName();
		Assert.assertNotNull(departments);
		for (Iterator iterator = departments.iterator(); iterator.hasNext();) {
			DepartmentEO departmentEO = (DepartmentEO) iterator.next();
			System.out.println(departmentEO);
	...
	public void findDepartmentsGreateLocId() {
		List<DepartmentEO> departments = departmentDAORef.findAllDepartmentsWhereLocationIdGreater(1700);
		Assert.assertNotNull(departments);
		for (Iterator iterator = departments.iterator(); iterator.hasNext();) {
			DepartmentEO departmentEO = (DepartmentEO) iterator.next();
			System.out.println(departmentEO);
